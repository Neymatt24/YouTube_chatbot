...
def get_groq_client():
    """
    Initialize Groq client with proper error handling for API key.
    """
    ...
    if not api_key:
        st.error("❌ Groq API key not found! Please set one of the following environment variables:")
        st.code("""
...
def main():
    ...
    try:
        ...
        st.error("❌ data.json file not found! Please ensure the file exists in the same directory.")
        st.stop()
    except Exception as e:
    ...
    # initialising Groq client with proper error handling
    ...
    if prompt := st.chat_input("Ask a Hydraulic Engineering question..."):
        ...
        system_prompt = (
            ...
            f"While answering queries involving mathematical symbols or equations, please and please ensure proper formatting of equations in LaTeX.\n\n"
